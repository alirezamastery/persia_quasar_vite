      ...
      <q-list>
        <template v-for="(item, i) in menuItems" :key="i">
          <q-expansion-item
            ...
            :header-class="
              $q.dark.isActive ? 'bg-grey-10 text-cyan-14' : 'bg-grey-2 text-cyan'
            "
          >
            <template v-for="(subItem, j) in item.children" :key="j">
              <q-item
                v-if="subItem.titleI18n !== 'car.cars'"
                :to="{ name: subItem.routeName }"
                :inset-level="0.3"
                clickable
                v-ripple
                :active-class="
                  $q.dark.isActive ? 'bg-blue-grey-10 text-cyan' : 'bg-blue-1 text-cyan'
                "
              >
              ...
              <template v-else>
                <q-item
                  v-if="userStore.mobile === '09358578419'"
                  :to="{ name: subItem.routeName }"
                  :inset-level="0.3"
                  clickable
                  v-ripple
